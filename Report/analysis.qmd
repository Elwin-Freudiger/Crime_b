...
Full_model <- lm(formula = Rate_per_1k ~ Density_2019 + Win_Lepen + 
                   ...
                   Unemp_2019 + Intensity_povrety + Total_pop, data = Crime_type)
```

First is our VIF analysis of the model.
```{r, echo = FALSE, message = FALSE}
knitr::kable(t(as.data.frame(vif(Full_model))), format = "html")
```

We then run a stepwise regression backwards to see what variables are not useful
