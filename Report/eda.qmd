...
```{r, message=FALSE, echo=FALSE, out.width= "100%", out.height= "50%"}
Bars
```
...
```{r, message=FALSE, echo=FALSE, fig.align='default', out.width=NULL, fig.show='asis'}
library(treemap)
treemap(Immig_tree, 
                index = "Reg_name", 
                vSize = "Immigrés", 
...
This map computes the unemployment percentage in every department, Unemployement is higher in the north and south as well as around Paris.
...
Lastly, this map shows us the immigration rate, by department. As we can see, Immigrants are mostly located in the vicinity of the Paris region. Other notable areas of higher immigration are border regions in the south and in the east. 

```{r, message=FALSE, echo=FALSE}
density_map
```

Here the density can be observed, with Paris and it's region being an outlier, it is hard to see the difference between other regions. This maps shows clearly that most people in France live around Paris.
...
We create a map using leaflet. To display another variable, simply click on the desired variable.
It should be noted that, for clarity reasons, outlier values for Paris regarding Crime rates and Density of population. Have been removed from the color scale. If this hasn't been done, it would be very difficult to see the differences in the remaining lower values.
...
In the above map, one can see the crime rate for many towns in France. To display them, simply click on the desired department.
It should be noted that not all towns are represented here. Furthermore, the circle center may be slightly off from the city center, this is because the coordinates used are the center of city limits, which often does not line up with the "economical" city center for example.
...
It also appears that the crime rate is very high in highly touristic places. Some places such as the winter sports resort town of Morzine or Saint-Tropez in the south have a low population during the year but see a large influx of tourists during the summer or winter. The criminality there is high because the total number of crimes is high compared to a low overall population.
Same observation for the communes where French airports are located such as Roissy-Charles-de-Gaulle Airport and the Lyon airport. All crimes reported in the airport are counted as committed inside the town, with millions of passengers each year, the crime rate is inflated.
...
We now compute the distance matrix. Using this distance matrix, we run a hierarchical clustering analysis using the average distance method. We get the following dendrogram:
...
We will run the same clustering 25 times with different centers as we initialize. Running it 25 times will ensure that we get the best clusters we can. 
