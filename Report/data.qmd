...
```{r, echo=FALSE, message=FALSE}
knitr::kable(head(Full_data_dep), format = "html")
```
...
As we can see, this dataset includes many towns of very few inhabitants. And many Missing values have been introduced into this dataset. As such, we will omit those missing values that were mostly located in the Poverty department.

We finally have this clean dataset:
```{r, echo=FALSE, message=FALSE, results='asis'}
knitr::kable(head(Crime_per_type_town), format = "html")
```

This clean dataset's smallest town is of `r min(Crime_per_type_town$Total_pop)`. This will be a much better dataset to run our regressions on. 
